...
def setup_gizmo():
    ...
    # Root group with a persistent text label (created once)
    root = displayio.Group()
    text_group = displayio.Group(scale=2, x=30, y=70)
    text_label = label.Label(terminalio.FONT, text="", color=0xFFD700)
    ...
    # Return both display and label so we can update text without rebuilding groups
    return d, text_label
...
## Note: This is a fixed version — updates existing label only.
def display_on_screen_speed(text_label, min_temp, max_temp, f, speed):
    cp.pixels.fill(0)  # Turns all of the NeoPixels off
    # displays the minimum, maximum, current temperature, and servo speed to the TFT Gizmo display.
    text = (
        "Min: " + str(round(min_temp, 2)) + " F\n"
...
def main():
    ...
    # Create the TFT Gizmo display and persistent label
    display, text_label = setup_gizmo()
    ...
    while True:
        # NOTE: We no longer rebuild display groups or root_group here.
        ...
        if cp.switch:
            # Blank out the text on the screen while using lights
            text_label.text = ""
            # Determines the number of neopixels to turn on
            display_as_lights(f_avg, min_scale_temp, max_scale_temp)
        else:
            display_on_screen_speed(text_label, min_temp, max_temp, f_avg, speed)
